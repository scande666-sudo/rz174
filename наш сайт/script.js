...
(() => {
  ...
  /* временный индикатор — чтобы видеть статус прямо на странице */
  const diag = document.getElementById('diag');
  const say = t => { if (diag) diag.textContent = 'сборка 20:25 · ' + t; };
  say('JS работает · Lenis: проверяю…');

  /* ---------- мега-плавный скролл ----------
     Lenis не подменяет скролл страницы трансформом: он каждый кадр
     подтягивает НАСТОЯЩУЮ позицию к целевой. Поэтому position: sticky
     и все расчёты через getBoundingClientRect работают как раньше.
     Библиотека лежит рядом, в vendor/ — интернет не нужен. */
  let lenis = null;
  if (typeof Lenis === 'undefined') {
    // библиотека не подгрузилась — страница просто крутится как обычно
    console.warn('[скролл] Lenis не найден, проверь что лежит vendor/lenis.min.js');
    say('JS работает · Lenis: НЕ НАЙДЕН');
  } else {
    // настройки взяты 1:1 с augen.pro (у них Lenis 1.0.39, те же цифры в бандле)
    lenis = new Lenis({
      duration: 0.8,                             // «длина полёта» после рывка колеса
      easing: t => 1 - Math.pow(1 - t, 4),       // quart-out: быстрый разгон, мягкая остановка
      smoothWheel: true,
      wheelMultiplier: 1,
      touchMultiplier: 1.8,
      syncTouch: false                           // на телефоне — родной скролл
    });
    window.__lenis = lenis;                      // для отладки из консоли
    say(`JS работает · Lenis: ОК (${reduced ? 'анимации выкл.' : 'анимации вкл.'})`);
    console.log('[скролл] Lenis включён. reduced-motion в системе:', reduced);

    /* ОТДЕЛЬНЫЙ цикл кадра ТОЛЬКО для Lenis.
       Он не зависит от тяжёлого frame() ниже. Даже если frame() где-то
       упадёт, прокрутка продолжает тикать — колесо не «залипнет». */
    const rafLenis = (time) => { lenis.raf(time); requestAnimationFrame(rafLenis); };
    requestAnimationFrame(rafLenis);
    console.log('[скролл] отдельный raf-цикл Lenis запущен');

    /* самопроверка: реально ли страница скроллится (есть ли высота) */
    setTimeout(() => {
      const h = document.documentElement.scrollHeight, vh = window.innerHeight;
      if (h > vh + 4) say(`OK · плавный скролл ВКЛ (высота ${Math.round(h)} > ${vh})`);
      else say(`Lenis ОК, но страница НЕ скроллится (высота ${h} ≤ ${vh})`);
    }, 600);
  }

  /* ---------- прелоадер: линейный счётчик, потолок 4 с ---------- */
  const pre = $('#pre'), preNum = $('#preNum'), preArc = $('#preArc'), preGem = $('#preGem');
  const T_MIN = 1700, T_MAX = 4000, t0 = performance.now();
  const RING_C = 2 * Math.PI * 92;                 // длина окружности кольца
  if (preArc) preArc.style.strokeDasharray = RING_C + ' ' + RING_C;
  ...
  const preTick = setInterval(() => {
    ...
    if (preArc) preArc.style.strokeDashoffset = (RING_C * (1 - p)).toFixed(2);
    if (preGem) {
      const g = clamp((p - 0.86) / 0.14);          // камень загорается на последних 14%
      preGem.style.opacity = (0.12 + 0.88 * g).toFixed(3);
      preGem.style.transform = `scale(${(0.5 + 0.5 * g).toFixed(3)})`;
      preGem.style.filter = `drop-shadow(0 0 ${(18 * g).toFixed(1)}px rgba(224,178,90,${(0.75 * g).toFixed(2)}))`;
    }
  ...
  function finishPre() {
    ...
    if (preArc) preArc.style.strokeDashoffset = '0';
    if (preGem) { preGem.style.opacity = '1'; preGem.style.transform = 'scale(1)'; preGem.style.filter = 'drop-shadow(0 0 18px rgba(224,178,90,0.75))'; }
  ...
  $$('a[href^="#"]:not([data-open])').forEach(a => a.addEventListener('click', e => {
    ...
    if (lenis) lenis.scrollTo(top);   // те же 0.8 c и та же кривая, что у augen.pro
  ...
  function frame(t) {
    ...
    /* --- футер: шина и буквы живут от курсора и скролла --- */
    {
      ...
      if (!frameErr) {
        frameErr = true;
        console.error('[скролл] frame() упал:', err);
        if (diag) diag.textContent = 'сборка 20:25 · ОШИБКА в frame(): ' + err.message;
      }
    }
